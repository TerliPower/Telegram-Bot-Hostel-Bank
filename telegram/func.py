from header import *
#from bot import get_text_mesages
...
def ExchangeExhibit(message):
    ...
    for currency_number in range(len(a)):
        if message.text == a[currency_number]:
            ...
            return 1
    MenuConstructor(message.from_user.id, [0], ExchangeExhibit, 'Такой валюты у вас нет! Кусь!', 'Купить', 'Выставить', 'Главное меню')
    return 1
    ...
    if message.text.isnumeric() == True:
        ...
        if amount <= queryam:
            ...
            if c >= b:
                ...
                counter = 0
                print(a[0], message.from_user.username, a[1], amount)
                if a[1][0] == '#':
                    print('exc occur')
...
def TransmitContracts(sender, receiver, contract, amount):
    with open('10contracts', 'r') as file:
        data = file.readlines()
    counter = 0
    i = -1
    for line in data:
        ...
        print(contractname, contract)
        if owner == sender and contractname == contract and counter < amount:
            print('find one')
            counter += 1
            linefrag = line.split(' ',1)[1]
            data[i] = (receiver + ' ' + linefrag)
